...
from backend.db.data_models import DAOPhotobooks, UserProvidedOccasion
...
class PhotobooksFullResponse(PhotobooksOverviewResponse):
    ...
    async def rendered_from_dao(
        ...
        pages = await DALPages.list_all(
            db_session,
            {"photobook_id": (FilterOp.EQ, photobook.id)},
        ...
        return PhotobooksFullResponse(
            **PhotobooksOverviewResponse.from_dao(photobook).model_dump(),
        ...
        async with self.app.new_db_session() as db_session:
            async with safe_commit(db_session):
                photobook = await DALPhotobooks.update_by_id(
                    db_session,
                    photobook_id,
                    DAOPhotobooksUpdate(title=payload.new_title),
                )
